...
async function fixture() {
  ...
  const [admin, investor, debtor, secondary] = await Promise.all(
  ...
  const market = await deploy(
    ...
    "InvoiceMarketplace",
    [await usd.getAddress(), admin.address],
  );
  const marketAddress = await market.getAddress();
  await (await asset.setKyc(marketAddress, true)).wait();
  await (
    await market.approveAsset(await asset.getAddress(), admin.address)
  ).wait();
  ...
  await (
    await usd.connect(investor).approve(marketAddress, 30_000_000n)
  ).wait();
  return {
    chain,
    admin,
...
test("atomic financing, compliant secondary transfer, and repayment to current holder", async () => {
  ...
  try {
    ...
    await (
      await f.asset.connect(f.investor).transfer(f.secondary.address, 1)
    ).wait();
    await (await f.usd.mint(f.debtor.address, 10_000_000n)).wait();
    await (
      await f.usd.connect(f.debtor).approve(f.marketAddress, 10_000_000n)
    ).wait();
    await (await f.market.connect(f.debtor).repay(1)).wait();
    await assert.rejects(f.market.connect(f.investor).claim.staticCall(1));
    await (
      await f.asset.connect(f.secondary).approve(f.marketAddress, 1)
    ).wait();
...
test("reject unauthorized issuer, duplicate listing, invalid terms, self-finance and non-seller cancellation", async () => {
  ...
  try {
    await assert.rejects(
      f.market.connect(f.investor).list.staticCall(...f.terms),
    );
    await assert.rejects(
      f.market.list.staticCall(...f.terms.slice(0, 3), 0, ...f.terms.slice(4)),
    );
    await (await f.market.list(...f.terms)).wait();
