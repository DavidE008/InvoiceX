...
export function seedInvoices(): Invoice[] {
  return [
    ...
    ensName: `inv-00${index + 1}.${label}.eth`,
    faceValue: String(faceValue),
    price: String(price),
    dueDate: new Date(Date.now() + Number(days) * 86_400_000)
      .toISOString()
      .slice(0, 10),
    status: "Open",
    seller,
    debtor: "0x1111111111111111111111111111111111111111",
    asset: `0x${String(index + 1).padStart(40, "0")}`,
    activity: [{ label: "Demo: Issued", at: new Date().toISOString() }],
  }));
}
