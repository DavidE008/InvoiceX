...
export function maximumInput(amount: bigint, slippageBps: number) {
  if (!Number.isInteger(slippageBps) || slippageBps < 1 || slippageBps > 300)
    throw new Error("Slippage must be between 0.01% and 3%.");
  return (amount * BigInt(10_000 + slippageBps) + 9_999n) / 10_000n;
}
export async function quoteTreasury(
  amount: string,
  slippageBps: number,
  config: Config,
): Promise<TreasuryQuote> {
  if (
    !isAddress(config.outputToken) ||
    config.outputToken.toLowerCase() === UNISWAP.weth
  )
    throw new Error(
      "Set a valid Sepolia output token, different from WETH, in Integrations.",
    );
  if (!/^\d+(\.\d+)?$/.test(amount) || Number(amount) <= 0)
    throw new Error("Enter a positive output amount.");
  ...
  if ((amount.split(".")[1]?.length ?? 0) > decimals)
    throw new Error(`Output token supports ${decimals} decimals.`);
  ...
  const available = quotes
    .flatMap((q) =>
      q.status === "fulfilled" && q.value.amountIn > 0n ? [q.value] : [],
    )
    .sort((a, b) => (a.amountIn < b.amountIn ? -1 : 1));
  if (!available.length)
    throw new Error(
      "No executable Uniswap v3 liquidity found for this amount on Sepolia. Try a smaller amount or another output token.",
    );
  return {
    ...available[0],
    tokenOut,
    amountOut,
    maxInput: maximumInput(available[0].amountIn, slippageBps),
    expiresAt: Date.now() + 120_000,
    decimals,
    symbol,
    rpc: config.sepoliaRpc,
  };
}
export async function executeTreasury(
  ...
  if (
    quote.tokenOut.toLowerCase() !== config.outputToken.toLowerCase() ||
    quote.rpc !== config.sepoliaRpc
  )
  ...
  const allowance = await client.readContract({
    ...
    args: [wallet.account.address, UNISWAP.router],
  });
  if (allowance < quote.maxInput)
    await confirmed(
      await wallet.writeContract({
        address: UNISWAP.weth,
        abi: erc20Abi,
        functionName: "approve",
        args: [UNISWAP.router, quote.maxInput],
      }),
      sepolia,
      config,
    );
  if (Date.now() >= quote.expiresAt)
    throw new Error(
      "Quote expired during wallet approvals. Wrapped ETH remains in your wallet; request a fresh quote.",
    );
    ...
    args: [
      {
        ...
        amountInMaximum: quote.maxInput,
        sqrtPriceLimitX96: 0n,
      },
    ],
  });
  const { request } = await client.simulateContract({
    ...
    args: [BigInt(Math.floor(quote.expiresAt / 1000)), [data]],
