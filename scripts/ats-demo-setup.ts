...
if (!/^0\.0\.\d+$/.test(assetId || ""))
  throw new Error("Provide an existing ATS testnet contract ID.");
...
if (!process.env.HEDERA_PRIVATE_KEY)
  throw new Error("Configure the ignored .env file.");
const signer = new Wallet(process.env.HEDERA_PRIVATE_KEY, provider);
const mirror = await fetch(
  `https://testnet.mirrornode.hedera.com/api/v1/contracts/${assetId}`,
);
...
const { SecurityRole } = require(
  resolve(root, "domain/context/security/SecurityRole.js"),
);
...
if (!(await asset.isInternalKycActivated()))
  throw new Error("Internal KYC must remain enabled.");
...
if (!existsSync(walletFile)) {
  ...
  writeFileSync(
    walletFile,
    JSON.stringify({ privateKey: investor.privateKey }, null, 2),
  );
}
const investor = new Wallet(
  JSON.parse(readFileSync(walletFile, "utf8")).privateKey,
  provider,
);
...
const evidence = existsSync(evidenceFile)
  ...
  : {
      ...
      kyc: "SYNTHETIC TESTNET FIXTURES ONLY — not identity verification",
      transactions: [],
    };
...
for (const name of ["_ISSUER_ROLE", "_SSI_MANAGER_ROLE", "_KYC_ROLE"]) {
  if (!(await asset.hasRole(SecurityRole[name], signer.address)))
    await record(name, asset.grantRole(SecurityRole[name], signer.address));
}
if (!(await asset.isIssuer(signer.address)))
  await record(
    "Register demo credential issuer",
    asset.addIssuer(signer.address),
  );
const targets = [
  signer.address,
  investor.address,
...
if (!targets[2])
  throw new Error("Set VITE_MARKETPLACE_ADDRESS to the deployed marketplace.");
for (const target of targets) {
  if (Number(await asset.getKycStatusFor(target)) !== 1) {
    await record(
      `Synthetic KYC ${target}`,
      asset.grantKyc(
        target,
        `invoicex:testnet:fixture:${target}`,
        0,
        1_830_297_600,
        signer.address,
      ),
...
console.log(
  JSON.stringify({
    ...
    assetAddress,
    investor: investor.address,
    supply: String(await asset.totalSupply()),
    internalKyc: true,
  }),
);
