...
await adapter.register(new Account({ id: accountId, evmAddress: signer.address }), true);
let result;
if (command === 'create') {
  ...
  result = await sdk.Bond.create(new sdk.CreateBondRequest({
    ...
    startingDate: String(Math.floor(Date.now() / 1000) + 120), maturityDate: String(Math.floor(Date.parse(input.maturityDate) / 1000)),
  ...
  if (!role) throw new Error('Use an exported ATS role name, for example _ISSUER_ROLE.');
  result = await sdk.Role.grantRole(new sdk.RoleRequest({ securityId: args[1], targetId: args[2], role }));
} else if (command === 'kyc') {
